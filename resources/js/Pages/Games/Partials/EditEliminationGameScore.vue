...
import {minValue, maxValue, integer, sameAs, not, helpers, requiredUnless} from '@vuelidate/validators'
...
const openModal = () => {
    ...
    axios.get(`/api/tournaments/${tournamentId}/elimination-games/${props.gameId}`)
        .then(response => {
            state.hostTeam = response.data.first_team.name;
            state.gameTime = response.data.game_time?.toString();
            if (response.data.team1_goals === 0) {
                state.hostTeamScore = '0'
            }else if (response.data.team1_goals) {
                state.hostTeamScore = response.data.team1_goals.toString()
            }else{
                state.hostTeamScore = '';
            }
            state.guestTeam = response.data.second_team.name;
            if (response.data.team2_goals === 0) {
                state.guestTeamScore = '0'
            }else if (response.data.team2_goals) {
                state.guestTeamScore = response.data.team2_goals.toString()
            }else{
...
const submitForm = () => {
    ...
    data.append('team1_goals', state.hostTeamScore);
    data.append('team2_goals', state.guestTeamScore);
    if (state.gameTime) {
        data.append('game_time', new Date(state.gameTime).toISOString());
    }
    axios.post(`/api/tournaments/${tournamentId}/elimination-games/${props.gameId}`, data, config).then(response => {
        emit('scoreUpdated');
        closeModal();
    })
        .catch(error => {
