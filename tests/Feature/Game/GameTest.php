...
class GameTest extends TestCase
{
    ...
    public function test_game_score_can_not_be_updated_if_wrong_game_id_is_provided(): void
    {
        $this->signInAdmin();
        $tournament = Tournament::factory()->create();
        Game::factory()->count(4)->create(['tournament_id' => $tournament->id]);
        $response = $this->patch(route('game.updateScore', ['tournament' => $tournament->id, 'game' => 'not-valid-game-id']),
            ['host_goals' => 2, 'guest_goals' => 0]
        );
        $response->assertStatus(404);
    }
    ...
    {
        $tournament = Tournament::factory()->create();
        Game::factory()->withScore()->count(4)->create(['tournament_id' => $tournament->id, 'fixture' => 1]);
        $response = $this->get(route('games.table', [
    ...
    public function test_game_score_table_with_valid_results_can_be_fetched(): void
    {
        ...
        Game::factory()->create([
            ...
            'host_goals' => 2,
            'guest_team_id' => $team2->id,
            'guest_goals' => 5,
        ]);

        Game::factory()->create([
            'tournament_id' => $tournament->id,
            'host_team_id' => $team2->id,
            'host_goals' => 2,
            'guest_team_id' => $team3->id,
            'guest_goals' => 0,
        ]);

        Game::factory()->create([
            'tournament_id' => $tournament->id,
            'host_team_id' => $team3->id,
            'host_goals' => 1,
            'guest_team_id' => $team4->id,
            'guest_goals' => 1,
        ]);
        ...
        $response->assertJsonFragment([
            'ID' => 1,
            'shorten_name' => $teamWithNegativePoints->shorten_name,
            'name' => $teamWithNegativePoints->name,
            'tournament_id' => $tournament->id,
            'image_path' => null,
            ...
            'Draws' => '0',
            'Losses' => '1',
            'Points' => (string) $teamWithNegativePoints->negative_points,
            'GamesPlayed' => 1,
            'Ranking' => 4
        ]);
